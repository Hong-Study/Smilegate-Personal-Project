...
#if defined(_MSC_VER)
	#define ROTL_ULONGK(x, n) _lrotl((x), (n))
	#define ROTR_ULONGK(x, n) _lrotr((x), (n))
#else
	#define ROTL_ULONGK(x, n) ((ULONGK)((x) << (n)) | (ULONGK)((x) >> (32 - (n))))
	#define ROTR_ULONGK(x, n) ((ULONGK)((x) >> (n)) | (ULONGK)((x) << (32 - (n))))
#endif

#define ENDIAN_REVERSE_ULONGK(dwS)	( (ROTL_ULONGK((dwS),  8) & 0x00ff00ff)	\
									| (ROTL_ULONGK((dwS), 24) & 0xff00ff00) )

#if defined(BIG_ENDIAN)
	#define BIG_B2D(B, D)		D = *(ULONGK_PTR)(B)
	#define BIG_D2B(D, B)		*(ULONGK_PTR)(B) = (ULONGK)(D)
	#define LITTLE_B2D(B, D)	D = ENDIAN_REVERSE_ULONGK(*(ULONGK_PTR)(B))
	#define LITTLE_D2B(D, B)	*(ULONGK_PTR)(B) = ENDIAN_REVERSE_ULONGK(D)
#elif defined(LITTLE_ENDIAN)
	#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_ULONGK(*(ULONGK_PTR)(B))
	#define BIG_D2B(D, B)		*(ULONGK_PTR)(B) = ENDIAN_REVERSE_ULONGK(D)
	#define LITTLE_B2D(B, D)	D = *(ULONGK_PTR)(B)
	#define LITTLE_D2B(D, B)	*(ULONGK_PTR)(B) = (ULONGK)(D)
#else
	#error ERROR : Invalid DataChangeType
#endif

#define RR(x, n)		ROTR_ULONGK(x, n)
#define SS(x, n)		(x >> n)
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Transform(ULONGK_PTR Message, ULONGK_PTR ChainVar)
{
	ULONGK a, b, c, d, e, f, g, h, T1, X[64];
	ULONGK j;
...
#define FF(a, b, c, d, e, f, g, h, j) {							\
	...
	#define GetData(x)	x
#else
	#define GetData(x)	ENDIAN_REVERSE_ULONGK(x)
#endif
...
void SHA256_Process( OUT SHA256_INFO *Info, IN const BYTE *pszMessage, IN UINT uDataLen )
{
	...
	while ((uDataLen + remain_buffer) >= SHA256_DIGEST_BLOCKLEN)
	{
		...
		SHA256_Transform((ULONGK_PTR)Info->szBuffer, Info->uChainVar);
...
void SHA256_Close( OUT SHA256_INFO *Info, IN BYTE *pszDigest )
{
	...
	Info->uLowLength = ENDIAN_REVERSE_ULONGK(Info->uLowLength);
	Info->uHighLength = ENDIAN_REVERSE_ULONGK(Info->uHighLength);
#endif

	((ULONGK_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((ULONGK_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
